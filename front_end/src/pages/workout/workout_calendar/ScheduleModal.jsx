...
const ScheduleModal = () => {
  ...
  const { memNo } = user;
  const { schedules, selectedDate, selectedSchedule, modalMode, setModalMode, showModal, setShowModal, setSignal } = useScheduleContext();
  const pastelColors = [
    '#76c3c5', // Pastel Green
    '#ff8d8d', // Pastel Red
  ...
  return (
    <>
      <Modal show={showModal} onHide={handleClose} centered fullscreen="sm-down">
        {' '}
        {/* 수정 내용: 모바일 화면에서 꽉 차게 설정 */}
            ...
            <Form.Group className="mb-3 flex flex-col gap-3">
              ...
              <div>
                <button
                  ...
                  className={`${allDay ? 'bg-teal-400 text-white border-teal-400' : 'bg-transparent text-gray-600 border-teal-300'} border-2 font-bold rounded-full text-sm w-full sm:w-20 h-9 text-center`} // 수정 내용: 모바일에서 버튼 너비 100%
                >
                  종일
                </button>
              </div>
