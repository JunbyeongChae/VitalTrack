...
const InfoBoardItem = ({ board}) => {
  ...
  return (
    <Link to={`/healthInfo/${board.infoNo}`} className="w-full aspect-square bg-white rounded-xl overflow-hidden shadow-lg border border-[#c2c8b0] hover:shadow-xl transition-all flex flex-col">
      ...
      <div className="p-4 flex flex-col justify-between h-1/2 text-[#5f7a60]">
        <h2 className="text-lg font-bold truncate">{board.infoTitle}</h2>
        <p className="text-sm">작성자 {board.memNick}</p>
        <p className="text-sm">작성일 {board.infoDate}</p>
        <p className="text-sm">조회수 {board.infoView} | 카테고리 {board.infoCategory}</p>
      </div>
    </Link>
  );
};
