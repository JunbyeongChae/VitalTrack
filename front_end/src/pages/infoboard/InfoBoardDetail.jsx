...
const postsData = [
  { id: 0, title: "📢 공지사항: 게시판 이용 안내", author: "관리자", date: "2025-02-28", views: 999, content: "이곳은 공지사항을 위한 공간입니다.", isNotice: true },
...
const InfoBoardDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-[#e3e7d3] flex flex-col items-center p-6 relative">
      <div className="max-w-3xl w-full p-6 bg-[#f2f5eb] text-[#5f7a60] rounded-xl shadow-lg border border-[#c2c8b0] mt-6">
        {/* 게시물 정보 */}
        <h1 className={`text-3xl font-semibold ${post.isNotice ? 'text-[#d97706]' : 'text-[#000000]'}`}>{post.title}</h1>
        <div className="text-[#000000] mb-4">{post.author} · {post.date} · 조회수 {post.views}</div>
        <div className={`p-6 rounded-lg shadow-sm mb-6 ${post.isNotice ? 'bg-[#fdf3c3]' : 'bg-[#d7e3c7]'} text-[#000000]`}>{post.content}</div>
        
        {/* 수정 및 삭제 버튼 */}
        <div className="flex justify-end space-x-2 mb-6">
          <button className="p-3 bg-white text-[#5f7a60] border border-[#a8b18f] rounded-md hover:bg-[#f2f5eb] transition-all shadow-sm">
            수정
          </button>
          <button
            onClick={handleDelete}
            className="p-3 bg-[#e5d8bf] text-[#5f7a60] border hover:bg-[#d7c7a8] rounded-md transition-all shadow-sm"
          >
            삭제
          </button>
        </div>
        ...
        <div className="mt-8 p-6 bg-[#f2f5eb] rounded-lg shadow-lg">
          ...
          <div className="flex items-end mb-4">
            <textarea
              ...
              className="w-5/6 p-3 border border-[#a8b18f] rounded-lg bg-white text-[#5f7a60] focus:ring-2 focus:ring-[#93ac90]"
              placeholder="댓글을 입력하세요..."
              rows="4"
              style={{ resize: 'none' }}
            />
            <button
              className="ml-4 p-3 bg-[#93ac90] text-white rounded-lg hover:bg-[#7c9473] transition-all shadow-md"
              disabled
            >
              댓글 등록
            </button>
