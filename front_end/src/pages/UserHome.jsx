...
const UserHome = () => {
  ...
  useEffect(() => {
    ...
    const todaySchedules = schedules.filter((schedule) => {
      const scheduleDate = schedule.start.split('T')[0]; // 'YYYY-MM-DD' 추출
      return scheduleDate === new Date().toISOString().split('T')[0];
    });
          ...
          <div className="p-6">
            ...
            <div className="mt-4 space-y-4">
              {todaySchedules.length > 0 ? (
                ...
                <p className="ml-3 text-sm text-gray-500">스케줄이 없습니다.</p>
              )}
            </div>
          </div>
        </div>
